...
This release cycle is the largest rules-automation expansion since the public beta began. It completes the shared runtime for Core, Offensive, Defensive, Stance, Preservation, Utility, Clash, Effect, Digizoid, Gain Force, Free and Negative Qualities, while preserving canonical IDs and compatibility with legacy or localized embedded Items.

Major additions include the rebuilt Clash engine, full Area Attack handling, the persistent Effect engine, Mode and Superior Mode Change, Conjurer/Summoner/Omnievoker support, compact Action consoles and a GM-only Token HUD palette for managing DDA Effects directly on Partner and NPC tokens.

The cycle also expands multiplayer-safe ownership routing, turn and round guards, action-economy enforcement, duration processing, attack binding, purchase validation, bilingual localization and dedicated DialogV2 interfaces throughout the system.

Compatibility and migration

Designed and validated for Foundry Virtual Tabletop v13.

No world migration is required for this release cycle.

Existing embedded Qualities continue to resolve through canonical IDs and supported legacy names.
...
Explicitly restored pointer events on the DDA Effect palette and its controls, overriding the disabled hit-testing inherited from the native Placeable HUD container.

Restored hover, keyboard focus, Foundry tooltips, left click, right click and Shift + left click in the live Foundry Token HUD.

Prevented pointer interaction from passing through the palette, selecting the Canvas or closing the Token HUD.

Kept the native Assign Status Effects control removed from all system Token HUDs.

Digimon Digital Adventures V2 — v2.0.19-beta.1

DDA40.1 — Compact and interactive Token HUD

Reduced the Effect palette width from 390 px to 300 px and the Effect controls from 34 px to 27 px without removing any of the 43 supported Effects.

Added native Foundry tooltips to every Effect, including its mechanical description, current state and interaction instructions.

Captured left click, right click and Shift + left click before native Token HUD handlers so the palette remains open while an Effect is changed.

Updated only the palette contents after Actor changes instead of rebuilding the complete Token HUD.

Removed the native Assign Status Effects control from Tamer, Partner, NPC and player-facing Token HUD views.

Digimon Digital Adventures V2 — v2.0.19-beta

DDA40 — DDA Effect palette for the Token HUD

Added a GM-only Effect palette to Partner and NPC Token HUDs with all 43 persistent DDA Effects grouped as Positive, Negative, Damage and Unique.

Added direct controls: left click applies or increases Duration, right click reduces or removes the Effect, and Shift + left click opens configuration for Duration, Potency/Value, Caster and [CONFUSE].

Stored Effect records in system.effects.active, sharing the same automation, sheet display, Combat Tracker and duration engine used by attack-applied Effects.

Integrated manual [HASTE] and [STUN] Action changes, [SHIELD] temporary Wound Boxes, [FEAR]/[TAUNT] replacement and [DOT] state tracking.

Corrected Section 9.02b handling for Negative Effects and Positive Effects applied to willing targets, including Health Pool and indiscriminate Area cases.

Audited Section 9.02c Damage behavior and corrected temporary Shield cleanup when the Effect expires at the start of its Caster's turn.

Bound Token HUD interaction only after the live ApplicationV2 DOM is mounted in Foundry v13.

Digimon Digital Adventures V2 — v2.0.18-beta.3

DDA39.3 — Post-render Action-card interactivity

Fixed Action cards that rendered correctly but did not execute their handlers.

Registered Action listeners against the mounted DOM only after DialogV2.render() resolves, matching the ApplicationV2 lifecycle.

Restored functional cards for Tamer, Partner Digimon and NPC Digimon Action consoles.

Digimon Digital Adventures V2 — v2.0.18-beta.2

DDA39.2 — Mandatory DialogV2 button configuration

Added the required non-empty buttons configuration expected by Foundry v13 DialogV2.

Hid only the API-required fallback footer while preserving Action cards, the window Close control and Escape behavior.

Restored the shared Tamer, Partner Digimon and NPC Digimon Action console after the DDA39.1 content-root correction.

Digimon Digital Adventures V2 — v2.0.18-beta.1

DDA39.1 — DialogV2 content-root hotfix

Corrected the shared Action-console content root to satisfy Foundry v13's attribute-free DialogV2 HTMLElement contract.

Restored Action-console access from Tamer, Partner Digimon and NPC Digimon sheets.

Guarded initialization and rendering failures so dialog errors are reported without creating unhandled Promise rejections.

Digimon Digital Adventures V2 — v2.0.18-beta

DDA39 — Compact Tamer and Digimon Action consoles

Replaced the two legacy Action dialogs with a shared Foundry v13 DialogV2 console.

Reduced Action cards to a responsive three-column layout with approximately 50 px card height.

Added Actor identity, remaining Actions, functional color accents and icons.

Moved descriptions into a contextual detail strip updated through hover or keyboard focus.

Collapsed Tamer Bolster and Interrupt notes into an optional Automatic Rules section.

Replaced large automation pills with compact Automated and Assisted indicators.

Removed the redundant full-width Close footer while retaining the window Close control and Escape behavior.

Preserved every Action handler, Action cost and dynamic Quality/Talent entry introduced through DDA38.

Digimon Digital Adventures V2 — v2.0.17-beta

Free and Negative Qualities

Completed the shared catalog and runtime integration for all 13 Free and 15 Negative Qualities on Partner Digimon and NPC Digimon.

Added round/turn/threshold automation for Violent Overwrite, Faulty Battery, Vengeful Charge, Sealed Weapon, Awakened Instinct, Positive Reinforcement and Boiling Point.

Added Critical Arms and Lucky Miss independent 2d6 results, including forced miss/hit, Disarm, Dodge override, Damage halving and Effect Potency/Duration reduction.

Added Low Vitality, Bulky, Vulnerable, Underwhelming, Broadside, Illness, Fumbled Piercing, Weakened Strike, Indiscriminate Targeting, System Error, Natural Weakness and Innate Talent integration over the existing derived-stat, pool, attack and area workflows.

Fixed Memory Upgrade attack slots and its exemption from the Free Quality limit.

Fixed Partner Wizard attack binding for Complex Signature, Fumbled Piercing and Weakened Strike; both Partner/NPC Wizards and both Quality Browsers use the same canonical definitions.

Effect duration audit

Normal durations tick once at the start of the Caster's turn; legacy source-less Effects tick on the Target instead of becoming permanent.

Special durations continue to resolve at the end of the appropriate activation, while Combat-bound Effects are removed on Combat end/delete.

Turn/round guards prevent duplicate processing from shared Partner/Tamer tracker units and multiplayer hooks.

Digimon Digital Adventures V2 — v2.0.16-beta

Digizoid Qualities and Gain Force

Added all Digizoid Armor, Digizoid Weaponry and Gain Force definitions with Stage, Rank, Algorithm and one-per-family purchase validation.

Completed conditional Armor/Weapon bonuses, Adaptive allocations, automatic successes, Light extra Action restrictions, Flexible Clash escape, Shining mitigation and Temporary IP.

Added Pure Digizoid [OFFHAND] Attack binding, Signature-only dual-Weapon handling, Rank 2 Weapon behavior and the 4 + Battery Accuracy/Damage cap.

Added Overwrite and Pure Overwrite Effect negation, Undying regeneration/Second Wind, Temporal Initiative control and interrupt suppression, and Omniscient self-Hold for Attack or Dodge.

Added independent Gain Force low-result rerolls and their Vital Energy, Huge Power and Avoidance rank interactions.

Added Digital Hazard and Zero Unit declaration workflows, Signature Battery interaction, GM-routed multiplayer mutations and Clash/evolution integration.

Extended Shining Armor mitigation to direct, Area, retaliatory and enemy Effect Unalterable Damage.

Compatibility

Includes every cumulative fix through DDA36.

No world migration is required; canonical IDs and legacy names remain supported.

tamer-actions.js, intercede.js and tamer-sheet.css remain byte-for-byte identical to DDA36. clash.js changes only for Flexible Digizoid Weaponry's automatic Escape the Clash.

Digimon Digital Adventures V2 — v2.0.15-beta

Mode Change, Superior Mode Change and Omnievoker

Completed paid/free Mode Change setup with one non-repeating Core Stat pair per Rank and an optional Stage-valid adjacent Size.

Added complete Superior Mode Change package configuration, exact DP matching, requirement checks, editable Mode Attack clones, Quality/Attack availability swaps, Wound Box delta handling and Resolve preservation.

Added the shared Mastery resource runtime for Conjurer and Summoner, including one-/two-Action access limits and one-Round cooldowns.

Added canvas Structures with Range/occupancy validation, Damage Threshold, linked multi-segment Wall/Terrain Wounds, DOS height metadata, replacement cleanup and Mastery refunds at 0 Wounds.

Added Infantry, Protector, Recon and Volatile Minions with official base Stats, sizes, Mastery upgrades, allowed inherited Qualities, Command restrictions, Summoner Attack consumption and shared Accuracy rolls.

Added Protector Intercede paid by the Summoner, Recon ranged values/owned-token vision, Volatile explosion and matching Element Master immunity, and Omnievoker's shared Action expenditure.

Added cleanup on replacement, source defeat and Quality loss.

Compatibility

Includes every cumulative fix through DDA35.

No world migration is required; canonical IDs and legacy names remain supported.

The four user-edited DDA35 files (tamer-actions.js, intercede.js, tamer-sheet.css, clash.js) are preserved byte-for-byte.

Digimon Digital Adventures V2 — v2.0.14-beta

Effect Qualities and Effect engine

Reworked the shared Effect engine around Sections 3.03–3.05 and 4.08: one Effect Tag per Attack, at most three purchasable Quality Tags, Positive Effects on Support Attacks only, self-target protection and the two-normal-Damage threshold for Damage Attacks.

Added purchase-time filtering for Basic, Advanced and Master Effects, including Stage requirements, no duplicate Effect purchases, legal Attack binding, Signature/Area compatibility and the permanent extra-Action exceptions for Haste, Paralyze, Weak, Strength and Stun.

Added Signature Support Battery Potency and the +1 Potency bonus for Melee Support Attacks.

Completed Potency, Resistance, Duration, reapplication and strongest-negative-per-Stat handling. Reapplying the same Effect extends its Duration without silently replacing the original Potency, except Fear/Taunt which correctly adopt the most recent Caster.

Completed Cleanse, including selective cleansing, allied Health Pools, Area Accuracy reduction, Resolve choice, Stun Action restoration and Shield cleanup.

Added Fear, Doom and Taunt resistance Checks both as a once-per-turn Action and at end of turn, with source-based TNs, mutual Fear/Taunt replacement and Area penalties.

Added allied and hostile Push/Pull scaling, Size reduction, canvas movement and Clash separation integration.

Added Immune duration reduction and Deny consumption; Cleanse remains exempt from Deny.

Added Shield usage tracking, Signature/Melee/Effect Warrior scaling, Doom absorption and temporary-Wound cleanup.

Added Inspiring Guidance, its rank cap based on the Positive Effect's DP cost, Area target choice, Guiding Dice pool spending and automatic cleanup when the source Effect ends.

Added the full Overclock purchase and execution flow, including the extra Effect DP cost, self-Resistance, Critical Failure lockout and exact normal/Critical duration timing.

Added DOT synthetic Attacks, original-Attack lockout, RAM Dodge bonus, Battery suppression and once-per-Combat target protection.

Added Stun delayed activation, Clash ending and Action restoration when removed early; Haste grants its Action and can bypass the Attack-per-Round rule.

Added the SV×2 per-Round cap for Effect Damage across turn damage and Burn movement damage.

Completed Heavy suppression ordering: Advanced Mobility before Extra Movement, and Transporter before Teleport.

Added Effect values and Resist controls to the Digimon sheet and dedicated DialogV2/chat-card styling.

Assisted boundaries

Blind continues to use the system's existing blinded-state penalties because the supplied glossary does not include the full sight/visibility procedure.

Push/Pull choose a legal canvas destination along the source-target line; unusual multi-level terrain, manual wall exceptions and occupied-space rulings remain GM-assisted.

The Attack sheet warns about illegal tag totals and the runtime blocks them, but existing legacy Attacks are not destructively rewritten.

Compatibility

Includes every cumulative fix through DDA34.

No world migration is required. Existing Effect Quality Items remain recognized through canonical IDs and legacy names; opening the Actor or Browser refreshes derived Effect metadata.

Static/module and focused mocked-rule validation passed. A real Foundry VTT v13 session with one GM and one player is still required before live play.

Digimon Digital Adventures V2 — v2.0.13-beta

Clash system and Clash Qualities

Rebuilt Clash initiation, control contests, tie-breaking, Size handling, Neutral Stance reset and the independent one-Clash-Action-per-turn economy around Section 9.08.

Added the complete Controller action set: Attack, Move, Pin, End Clash, Throw, Extended Grapple Pull and Wrestlemania Finisher.

Added the complete Opponent action set: Weak Attack, Clash Comeback, Contest Pin, Escape and Teleport.

Added automatic control contests at the start of the initiator's turn, Pin's automatic-control rule, Comeback/Contest Pin bonuses and the three-Pin-per-Combat limit.

Integrated attacks from outside a Clash: no Dodge Pool, combined CPU Damage reduction, combined RAM synthetic Dodge for Support Attacks and Exposing Hold's reduced protection.

Added outside intervention: an adjacent third Digimon may spend 2 Actions to pit CPU Checks against the Controller's RAM and pry the Clash apart.

Integrated Clash separation through PUSH/PULL, Fear caused by the other participant, Stun, Paralyze and movement beyond Extended Grapple reach.

Implemented Monster Strength, Exposing Hold, Point Blank, Slippery, Fastball, Giant Hijacker, Titan Power, Distant Force and Power Throw.

Added Monster Strength dragging, Titan Power Size bypass, Point Blank ranged Clash attacks, Slippery RAM×2 checks and persistent Giant Hijacker rider/host movement.

Added Fastball preparation and owner-routed [CHARGE] resolution, including one- or two-Action investment, attack-limit transfer and Power Throw/Tumbler Crash protection.

Added Distant Force contested movement and Clash hold checks when forced movement would separate participants.

Added offensive Throw Accuracy from CPU, Power Throw Range/Damage bonuses and controlled post-Throw movement.

Added dedicated DialogV2 and chat-card styling for Clash contests, actions and Qualities.

Cleaned the English Clash Quality records that still contained mixed-language requirements, result text or section bleed.

Assisted rulings

Collision and falling Crash Damage remain assisted because the supplied Clash section does not include the underlying Crash Damage formula or enough geometry to determine actual collision distance.

Throwing an Opponent into a group remains assisted for placement of the unique Size-based Area; single-target offensive Throws are automated.

Finisher's aerial Crash option and GM-approved custom Called Shot effect remain assisted by design.

Fastball exposes the one-Action Area Intercede cost to the automation API, but the base Area Intercede procedure is not present in the supplied Clash rules and is not invented here.

Compatibility

Includes all cumulative fixes and Stance, Preservation and Utility automation from DDA33.

No Actor or world migration is required. Existing embedded Clash Qualities are recognized through canonical IDs, English names and legacy PT-BR names.

Static and mocked-module validation passed; a real Foundry VTT v13 session with one GM and at least one player is still required before live play.

Digimon Digital Adventures V2 — v2.0.12-beta

Stance, Preservation and Utility Qualities

Implemented Fierce Soul, Brave Heart and Sentry Aim as selectable Stances integrated with the attack, Intercede, movement and Area Attack pipelines.

Added Fierce Stance stat changes and its once-per-turn same-Attack retry, including Battery preservation and the all-target miss requirement for Area Attacks.

Added Brave Stance Armor/Movement changes and the post-Intercede temporary Damage bonus based on nearby Allies.

Added Sentry Zone placement and movement, enter/exit Interrupt Attacks, one-hit-per-target-per-Round protection, ranged Accuracy immunity, melee Dodge penalty, close-target restriction and Difficult Terrain movement.

Implemented Second Wind, Pack Master integration, Vital Energy rerolls, Tumbler Crash reduction, Focused Resistance and Immunity.

Added the complete System Boost first-purchase discount and per-Rank Derived Stat selection.

Implemented Teleport and Transporter movement, attack/Clash escape, visible free-space validation and next-turn Action debt.

Added Glamor target selection, shared appearance state, optional place swap, hit/size break conditions and creator cleanup.

Added Illusionary Overlay templates, Naturewalk-element Shrouds, source-specific Blind application, range cleanup and assisted Barrier handling.

Implemented all twenty Domain Control options, Adaptive Element, Altered Element, moving Domain templates, duration/maintenance, target selection, terrain and attack modifiers.

Implemented Overdrive, Data Scan, Holy Ward, Dark Emblem and Chaotic Balance, including the special purchase exception unlocked by Chaotic Balance.

Added dedicated DialogV2 styling for all new Stance, Preservation and Utility windows.

Assisted rulings

Illusionary Barriers remain assisted because each creature can independently believe or disbelieve the same barrier; the system records the template and TN without creating a universal physical Wall.

Glamor cover failures, physical grabs and deliberate Awareness challenges remain GM-resolved, while the persistent state and automatic break conditions are tracked.

Surprise Round setup is not invented by this update; Combat Awareness contributes its Initiative bonus and the existing treatsSurpriseRoundsAsNormal flag for the system's Surprise workflow.

Compatibility

Includes all cumulative fixes and Defensive Quality automation from DDA32.

No Actor or world migration is required. Existing embedded Quality Items are recognized through canonical IDs and legacy names.

Runtime verification is still required in Foundry VTT v13 with one GM and at least one player client before using the update in a live session.

Digimon Digital Adventures V2 — v2.0.11-beta

Defensive Qualities

Implemented the complete Starting, Champion and Ultimate Defensive Quality set: Absolute Evasion, Avoidance, Combat Monster, Bullet Proof, Substitute, Brace, Savagery, Assured Destruction, Berserker and Boiling Blood.

Added canonical and legacy-name matching, rank and Stage validation, total-Dodge requirements and two-way incompatibility validation between Absolute Evasion and Data Specialization: Uncatchable Target.

Rebuilt Dodge preparation so Absolute Evasion converts base dice into automatic Successes and consecutive Dodge penalties consume those Successes before reducing the remaining Pool.

Added post-roll Avoidance rerolls for eligible natural dice while preserving bonus dice granted by Tamer Actions.

Added a complete Resolve lifecycle: true-Wound damage from enemies and enemy-sourced ongoing Effects, maximum 4/6, successful-Attack expenditure, CLEANSE reduction, Savagery generation, Berserker expansion, Boiling Blood initialization and turn-start recovery, plus combat-end cleanup.

Added per-attacker Bullet Proof stacking, Accuracy and Damage penalties, Armor-to-zero permission and reset at the defender's own end turn.

Added owner-routed Substitute and Brace reactions before damage application, including escalating TNs, action payment, critical outcomes, Intercede interaction and Clash-escape support.

Added Savagery and Assured Destruction declaration flows to the attack pipeline, including Doom-aware temporary Wound Boxes and Resolve-to-Accuracy conversion.

Added primary-GM ownership for persistent combat updates and idempotent initialization when a Combat starts or gains a new Combatant.

Styled all new defensive prompts with DialogV2 and the system's current dark-blue/cyan interface language.

Compatibility

Includes all cumulative fixes and Offensive Quality automation from DDA31.

No Actor or world migration is required. Existing embedded Quality Items are recognized through canonical IDs and legacy names.

Runtime verification is still required in Foundry VTT v13 with one GM and at least one player client before using the update in a live session.

Digimon Digital Adventures V2 — v2.0.10-beta

Offensive Qualities

Implemented the Starting, Champion and Ultimate Offensive Quality set with canonical IDs, legacy-name aliases and safe automation defaults for embedded Items.

Added purchase and binding validation for Armor Piercing, Certain Strike, Ammo, Venomous, Mighty Blow, Feint Attack, Lifesteal, Reach and all attack-specific Tags.

Completed the attack pipeline for [PIERCING], [CERTAIN], [CHARGE], [AMMO], [RECOIL], [SNEAK], [SIMPLE], [VENOM], [FOCUS], [T:MIGHTY], [T:FEINT], [PUNISH], [COUNTER] and [DRAIN].

Added action interfaces for Horde Duelist, Hide in Plain Sight, Shade Cloak, Battle Cry, Watchful Hunter, Reload, Counterattack variants and Punishing Strike reactions.

Expanded Area Attacks to all six templates, final-damage batching, Zoner options, Reach interactions, T movement and Heavy Recoil size restrictions.

Added Extended Grapple range and the Pull Primary Clash Action.

Expanded Coordinated Assault maintenance and termination rules.

Migrated all new and updated Offensive Quality prompts to DialogV2 with the system's current visual language.

Assisted rulings

Hide in Plain Sight requests a GM-entered Stealth TN because Section 9.09b was not included with this rules batch.

T wall bounces and extraordinary collision/path cases remain GM-assisted rather than guessing geometry through solid scene obstacles.

Compatibility

Includes all cumulative fixes from DDA30.

No Actor or world migration is required; legacy embedded Offensive Quality Items receive missing automation data when prepared.

Digimon Digital Adventures V2 — v2.0.9-beta

Emergency Attack Roll Fix

Fixed the duplicate getCombatantForActor binding that prevented attack-roll.js from loading.

Standardized Attack Roll combatant lookups on the shared Initiative helper signature: getCombatantForActor(combat, actor).

Passed the Area Attack batch context into attack quality notes, preventing a runtime error while resolving True Guardian protection.

No Actor, Item or world data migration is required.

Digimon Digital Adventures V2 — v2.0.8-beta

Core Qualities

Rebuilt Starting, Champion and Ultimate Core Quality automation around stable canonical IDs, preserving compatibility with localized and legacy Actors.

Implemented the collective Core Discount across all Core Qualities and corrected Elemental Myriad eligibility.

Added strict purchase validation for Stage, Rank, prerequisites, incompatible Qualities, unique choices, Algorithm, Advanced Mobility, Data Specialization and Hybrid Drive.

Added the Core Build Review dashboard with discount accounting, build warnings and safe repair of missing Data Specialization grants.

Added free Quality grants and conversion of paid Ranks for Fistful of Force, Mobile Artillery, Try Something, Wrestlemania, Hit and Run and Tactical Adaptation.

Expanded combat integration for Data Optimization, Naturewalk, Weapon, Instinct, Sprint, Elemental Force, Element Master and all Data Specializations.

Added persistent Mobile Artillery Difficult Terrain, automatic Surface movement cost, Naturewalk immunity and start-of-next-turn cleanup.

Added polished DialogV2 interfaces and missing English/Brazilian Portuguese strings.

Compatibility

Keeps the DDA28 alternating Initiative order and the DDA27 final Area Damage card.

Existing Actors can be repaired from the Quality Browser without spending DP or duplicating granted Ranks.
