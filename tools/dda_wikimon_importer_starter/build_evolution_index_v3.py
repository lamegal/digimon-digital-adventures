...
OUTPUT_PATH = Path("output/all_digimon_with_evolution_index_v3.json")
REPORT_PATH = Path("output/evolution_index_report_v3.json")
UNRESOLVED_PATH = Path("output/evolution_index_unresolved_v3.csv")
...
def add_spacing_variants(alias_map, alias_conflicts, candidate, actor):
    text = norm(candidate)
    if not text:
        return
    ...
    # Variante simples para nomes muito comuns com espaço removido.
    if " " in text:
        variants.add(text.replace(" ", ""))

    for variant in variants:
        add_alias(alias_map, alias_conflicts, variant, actor)
...
def build_alias_map(actors):
    ...
    for actor in actors:
        ...
        for candidate in candidates:
            add_spacing_variants(alias_map, alias_conflicts, candidate, actor)
...
def should_keep_unresolved(entry, target):
    ...
    for actor in actors:
        ...
        for direction in ("From", "To"):
            ...
            for entry in iter_entries(actor, direction):
                ...
                if not other_actor:
                    if should_keep_unresolved(entry, target):
                        row = {
